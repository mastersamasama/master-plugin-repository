...
Claude 会自动生成 `.zip` 文件。然后把 zip 文件和你的 Demo（录屏或在线链接）上传到 [提交表](https://okg-block.sg.larksuite.com/share/base/form/shrlg8ofWhLJotSJdkIaQy9PxMg) 就行了。不需要 GitHub 账号，不需要 PR，不需要 `gh` CLI。
