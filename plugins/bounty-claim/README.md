...
cd BountyClaim/scripts

# 1. Copy the env template and fill in your keys
cp .env.example .env

# 2. Install all dependencies (installs Python packages via postinstall hook)
npm install
...
> - `OKX_API_KEY`, `OKX_SECRET_KEY`, `OKX_PASSPHRASE`, `OK_PROJECT_ID` — OKX Onchain OS credentials
> - `PRIVATE_KEY` — EVM private key of the account that pays out USDC
